...
from google_news_fallback import fetch_all as fetch_google_news
from md_writer import write_all
from news_fetch import fetch_all as fetch_rss
from news_filter import filter_entries


def collect_entries() -> list[dict]:
    rss_entries = fetch_rss()
    print(f"[run_daily_news] RSS 수집: {len(rss_entries)}건")

    try:
        google_entries = fetch_google_news()
    except Exception as exc:  # 비공식 우회라 언제든 깨질 수 있음 — RSS 수집까지 잃지 않는다.
        print(f"[run_daily_news] 경고: Google News 수집 실패, RSS만 사용합니다: {exc}")
        google_entries = []
    print(f"[run_daily_news] Google News 수집: {len(google_entries)}건")

    return rss_entries + google_entries


def main() -> None:
    raw_entries = collect_entries()
    print(f"[run_daily_news] 전체 수집: {len(raw_entries)}건")
